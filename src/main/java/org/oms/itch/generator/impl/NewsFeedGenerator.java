...
public class NewsFeedGenerator implements ITCHMessageGenerator {
    Random random;
    String[] messageTypes = { "T", "S", "L", "M", "P", "R", "H", "N" }; // ITCH Index
    ...
    private String generateDummyText(int length) {
        ...
        char[] rawString = new char[length];
        for (int i = 0; i < length; i++) {
            int randomIndex = random.nextInt(ALPHANUMERIC_CHARS.length());
            char randomChar = ALPHANUMERIC_CHARS.charAt(randomIndex);
            // sb.append(randomChar);
            rawString[i] = randomChar;
        }
        return new String(rawString);
    }

    private ByteBuffer generateNewsItem() {
        ...
        buffer.put(String.format("%-" + 30 + "s", companyDetail[2]).getBytes());
        buffer.put(generateDummyText(random.nextInt(10, 81)).getBytes());
        buffer.put((byte) 0);
        buffer.put(generateDummyText(random.nextInt(10, 201)).getBytes());
        buffer.put((byte) 0);
        buffer.put(generateDummyText(random.nextInt(10, 512)).getBytes());
        buffer.put((byte) 0);
        int currentPosition = buffer.position();

        // Set the limit to the current position and reset the position to 0
        buffer.flip();

        // Create a byte array with only the valid data
        byte[] strippedByteArray = new byte[currentPosition];
        buffer.get(strippedByteArray);

        return ByteBuffer.wrap(strippedByteArray);
    }
